...
main() {
  ...
  int qq_,*hh,qf,(*g4)(void*)=&fflush,(*z1)(const char*,...)=&printf,(*p0)(int)=&putchar,
      c = 0,i,dd, sz,h, s, e,pr,*trundle,*vp5,*nqd,oo[10];
  char *p5;
goto cow;  
pic:g4(y3);
  sz = dd*h;
...
{      int x=oo[2],y=oo[3],dd=*oo,h=oo[1],i=x+y*dd;
      ...
      if (x-1>-1&&(p5[i-1]==_int[66]||p5[i-1]==_int[67]))chew: oo[5+oo[4]++]=i-1;
      if (y+1<h&&p5[i+dd]==_int[67]||p5[i+dd]==_int[66])oo[5+oo[4]++]=i+dd;
      if (y-1>-1&&p5[i-dd]==_int[67]||p5[i-dd]==_int[66])chun: oo[5+oo[4]++]=i-dd;
    }
    if (c&1?nqd[c]:vp5[c]) goto tokyo;
wow:;
    if (oo[4]--<=0) goto chin;
      i=oo[5+oo[4]];
